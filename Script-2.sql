...
select a.first_name, a.last_name, count(f.rental_duration) as count from public.actor a
...
select c.name, sum(f.rental_rate) as sum from public.category c 
join public.film_category fc on c.category_id = fc.category_id
join public.film f on f.film_id = fc.film_id 
group by c.name
order by sum desc
...
select * from (
    select a.first_name, a.last_name, count(f.film_id) as count, dense_rank() over(order by count(f.film_id) desc) as rank from public.actor a 
    join public.film_actor fa on fa.actor_id = a.actor_id 
    join public.film f on f.film_id = fa.film_id 
